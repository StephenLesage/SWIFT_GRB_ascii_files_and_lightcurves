...
for i in range(len(SWIFT_ascii_files)):
    ...
    #finding 10% so the plot can show 120% of the burst
 	ten_percent = (t_100_stop-t_100_start)/10.0

    #find where elements of time array are greater than t_100_start minus 10% and less than the t_100_stop time plus 10%
	beginning_section = [i for i,v in enumerate(time) if v >= t_100_start-ten_percent]
	adjusted_time = [j for j,v in enumerate(time) if v <= t_100_stop+ ten_percent]
	
    #getting the indices for t_100_start minus 10% and t_100_stop plus 10%
